...
class SpellModel(BaseModel):
    ...
    def check_index(cls, index):
        if 0 < index < 100:
            return index
        raise ValueError("Індекс повинен бути у діапазоні від 0 до 100")

    @field_validator("spell")
    def check_spell(cls, spell):
        if len(spell) > 3:
            return spell.upper()
        raise ValueError("Заклинання повинно бути довшим за 3 символи")
...
class UserModel(BaseModel):
    ...
    def check_first_name(cls, first_name: str):
        if all([first_name.isalpha(), len(first_name) >= 2]):
            return first_name
        raise ValueError("Ім'я повинно мати мінімум 2 символи, лише літери")
