...
async def get_spell(index: int):
    ...
    if not spell:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Заклинання з id {index} не знайдено")
    return spell
...
#     db.append(new_spell)
#     save_db(db)
#     return "Нове заклинання успішно додане"
...
async def add_spell(spell_model: SpellModel):
    ...
    save_db(db)
    return JSONResponse("Нове заклинання успішно додане")
