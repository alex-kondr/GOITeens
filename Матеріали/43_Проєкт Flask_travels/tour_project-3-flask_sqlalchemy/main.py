...
from data.models_old import Tour, User, tour_user_assoc
...
@login_required
def cabinet():
    tours_id = [tour.id for tour in current_user.tours]
    next_page = request.args.get("page", 1)
    tours_page = db.paginate(db.session.query(Tour).filter(Tour.id.in_(tours_id)), per_page=2, page=int(next_page))
    return render_template("cabinet.html", tours_page=tours_page)
