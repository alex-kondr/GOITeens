...
while True:
    ...
    if command == "1":
        for client in clients:
            print(f"Клієнт '{client[0]}' перебуває в кімнаті № {client[-1]}")

        input("Натисніть Enter для продовження ")

    if command == "2":
        client_input = input("Введіть кого висилити (Ім'я та Приізвищен): ")
        rating = input("Введіть оцінку обслуговування: ")

        for client in clients:
            if client_input in client[0]:
                clients.remove(client)
                client.append(rating)
                rating_customers.append(client)
                break

        input(f"Клієнт '{client[0]}' виїхав з кімнати № {client[-2]} та виставив такий рейтинг: {client[-1]}\nНапистіть Enter для продовження ")
